...
export function createServer({cacheStore=cache,probe=probeUrl}={}){
 return http.createServer(async(req,res)=>{res.setHeader('x-request-id',req.headers['x-request-id']||crypto.randomUUID());const u=new URL(req.url,'http://preflight.local');try{
   ...
   if(req.method==='GET'&&u.pathname==='/')return send(res,200,{name:'PREFLIGHT',version:VERSION,tagline:'Before your AI agent visits a URL, ask us how to get there.',check:'GET /v1/check?url=https://example.com',routes:['HTTP','BROWSER','MACHINE_ENDPOINT','AVOID']});
  ...
  }catch(e){const status=e?.code==='PREFLIGHT_UNSAFE_URL'?422:e?.code==='PREFLIGHT_BAD_URL'?400:500;return send(res,status,{error:e instanceof Error?e.message:'Unknown error',code:e?.code||'PREFLIGHT_INTERNAL'});}});
}
